...
fit.without.polynomials<-lm(age~termstart+chamber+party+state+GDP+incumbent,data=congress)
fit.2.termstart<-lm(age~poly(termstart,2)+chamber+party+state+GDP+incumbent,data=congress)
fit.2.gdp<-lm(age~termstart+chamber+party+state+poly(GDP,2)+incumbent,data=congress)
fit.3.termstart<-lm(age~poly(termstart,3)+chamber+party+state+GDP+incumbent,data=congress)
fit.3.gdp<-lm(age~termstart+chamber+party+state+poly(GDP,3)+incumbent,data=congress)
fit.4.termstart<-lm(age~poly(termstart,4)+chamber+party+state+GDP+incumbent,data=congress)
fit.4.gdp<-lm(age~termstart+chamber+party+state+poly(GDP,4)+incumbent,data=congress)
fit.5.termstart<-lm(age~poly(termstart,5)+chamber+party+state+GDP+incumbent,data=congress)
fit.5.gdp<-lm(age~termstart+chamber+party+state+poly(GDP,5)+incumbent,data=congress)
...
##GAMs
fit<-gam(age~s(termstart,df=2)+chamber+party+state+s(GDP,df=2)+incumbent,data=congress.train)
...
cvs<-rep(NA,15)
for(i in 5:15){
  fit<-glm(age~ns(termstart,df=i)+chamber+party+state+GDP+incumbent,data=congress)
  cvs[i]<-cv.glm(congress,fit,K=10)$delta[1]
}
fit<-lm(age~ns(termstart,df=8)+chamber+party+state+GDP+incumbent,data=congress)
summary(fit)
#this gives a multiple r2 of 0.1121

#specified splines on the decade
fit<-lm(age~ns(termstart,knots=c(1970,1980,1990,2000))+chamber+party+state+GDP+incumbent,data=congress)
summary(fit)
