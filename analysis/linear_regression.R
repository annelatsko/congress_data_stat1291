...
lm.fit.incumbent <- lm(age~incumbent, data=congress)
...
lm.fit.og <- lm(age~termstart+chamber+party+state+incumbent, data=congress) #r2 = 0.08795
...
lm.fit.important <- lm(age~termstart+chamber+party+state+GDP+incumbent, data=congress)
summary(lm.fit.important)
